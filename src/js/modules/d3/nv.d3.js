...
nv.models.multiChart = function() {
	...
	var showTooltip = function(e, offsetElement) {
		   ...
		   eleSum
			...
			.style('fill',function(d,i){return d.color || color[(i+1) % color.length];});
	...
	function chart(selection) {
		selection.each(function(data) {
			...
			}).concat([{x:0, y:0},{x:0,y:1},{x:0,y:-1}]) : [{x:0, y:0},{x:0,y:1}]
...
*/
			...
			lines1.yDomain(yScale1.domain())
			  .margin({top: 0, right:availableWidth/(2*data[0].values.length), bottom: 0, left: availableWidth/(2*data[0].values.length)});
...
*/
			...
			yAxis2
				.tickValues(d3.range(y2Min, y2Max,(y2Max-y2Min)/5));
			yAxis2
				.tickSize( 0, 0);
			d3.transition(g.select('.y2.axis'))
				.call(yAxis2);
