...
(function() {
	...
	nv.tooltip.show = function(pos, content, gravity, dist, parentContainer, classes) {
		...
		//Create new tooltip div if it doesn't exist on DOM.
		// console.log('in nv.tooltip.show!!!!!!!!!!!!!!!!!!!!!!!!!!!!!');
...
nv.models.line = function() {
	...
	var margin = {top: 0, right: 14, bottom: 0, left: 16}
		, width = 960
		, height = 500
		...
		selection.each(function(data) {
			...
			var barsEnter = bars.enter().append('rect')
					...
					.attr('x', function(d,i,j) {
						return stacked ? 0 : (j * x.rangeBand() )
					})
					...
					.each(function(d,i,j){
						if(d.background){
							...
							.attr('x', stacked ? 0 : (j * x.rangeBand() / data.length ))
							.attr('y',  0 )
							.attr('height', availableHeight)
							.attr('width', (x.rangeBand()/(1-groupSpacing)))
							.attr('fill','black')
							.attr('transform', 'translate(' + x(getX(d,i)) + ',0)')
						}
					})
				;
			bars
			...
			bars
				.attr('class', function(d,i) { return ((getY(d,i) < 0 ? 'nv-bar negative' : 'nv-bar positive')+' ele ele-'+i)})
				.attr('transform', function(d,i) { return 'translate(' + x(getX(d,i)) + ',0)'; })
...
nv.models.multiBarChart = function() {
	...
	var showTooltip = function(e, offsetElement) {
		  eleSum = eles.filter(function(d,i){
		   ...
		   // console.log('barSum',eleSum);
		   eleSum
			.classed({'selected':true,'unselected':false});
		   above
			.classed('above',false);
	...
	dispatch.on('tooltipHide', function() {
		eles
		   .classed({'selected':false,'unselected':true})
		above
			.classed('above',true);
		if (tooltips) nv.tooltip.cleanup();
	});
...
nv.models.multiChart = function() {
	...
	var margin = {top: 30, right: 20, bottom: 50, left: 60},
		...
		tooltip = function(date,arg0, arg1, arg2, e, graph) {
			return '<h3>' + date + '</h3>' +
				'<p class="pro">总报名人数</p>'+
				'<p class="num">' + arg0 + '</p>'+
				'<p class="pro">新增注册人数</p>'+
				'<p class="num">' + arg1 + '</p>'+
				'<p class="pro">新增退课人数</p>'+
				'<p class="num">' + arg2 + '</p>';
		},
		x,
		y,
	...
	var x = d3.time.scale(),
		...
		xAxis = nv.models.axis().scale(x).orient('bottom').tickPadding(1),
		yAxis1 = nv.models.axis().scale(yScale1).orient('left'),
		yAxis2 = nv.models.axis().scale(yScale2).orient('right'),

		legend = nv.models.legend().height(30),
		dispatch = d3.dispatch('tooltipShow', 'tooltipHide'),
		eles,elesIn,eleRe,eleSum,circles,rects;
	var showTooltip = function(e, offsetElement) {
			eleSum = eles.filter(function(d,i){
				return (d.id==e.point.id);
			})
			// circles
			//     .filter(function(d,i){
			//         return (d.id==e.point.id);
			...
			// console.log(eleSum,'elesum');
			eles
				.classed('selected',false);
		   eleSum
			.classed('selected',true);
			// console.log('show',eleSum.select('circle'));
			...
			eleSum.each(function(d,i){
				arg0=arg0+(i==2?d.y:'');
				arg1=arg1+(i==0?d.y:'');
				arg2=arg2+(i==1?Math.abs(d.y):'');
			})
		var left = e.pos[0] + ( offsetElement.offsetLeft || 0 ),
			top = e.pos[1] + ( offsetElement.offsetTop || 0) + (document.body.scrollTop||document.documentElement.scrollTop||0),
			//x = xAxis.tickFormat()(lines1.x()(e.point, e.pointIndex)),
			y = arg1+arg2+((e.series.yAxis == 2) ? yAxis2 : yAxis1).tickFormat()(lines1.y()(e.point, e.pointIndex)),
	...
	function chart(selection) {
		selection.each(function(data) {
			...
			var rang1 = d3.extent(d3.merge(series1).concat(extraValue1), function(d) { return d.y } );
			var rang2 = d3.extent(d3.merge(series2).concat(extraValue2), function(d) { return d.y } );
			var y1Min = rang1[0];
			var y1Max = rang1[1];
			var y2Max = rang2[1];
			var y2Min = rang2[0];
			//console.log(y1Min,y1Max,y2Min,y2Max)
			y1Min = y2Min*y1Max/y2Max;
			// yScale1 .domain(yDomain1 || d3.extent(d3.merge(series1).concat(extraValue1), function(d) { return d.y } ))
			//     .range([0, availableHeight])
			yScale1 .domain(yDomain1 || [y1Min,y1Max])
				.range([0, availableHeight]);
			...
			yScale1 .domain(yDomain1 || [y1Min,y1Max])
				.range([0, availableHeight]);
			var factor = 200;
			while(((y1Max)/factor-y2Max)<0)
				factor = factor/2;
			y2Max = y1Max/factor;
			y2Min = y1Min/factor;
			yScale2 .domain(yDomain2 || [y2Min,y2Max])
				.range([0, availableHeight]);

			// var ticks2 = yScale2.ticks();
			// var minus = ticks2[1] - ticks2[0];

			// y2Max = ticks2[ticks2.length-1] + minus;
			// y2Min = y1Min*y2Max/y1Max;
			// yScale2 .domain(yDomain2 || [y2Min,y2Max])
			//     .range([0, availableHeight]).ticks(ticks)
			...
			d3.transition(g.select('.x.axis'))
				.call(xAxis);
			var arr1 = d3.range(y1Min, y1Max,(y1Max-y1Min)/multipleNum);
			// console.log(arr1);
			...
			yAxis2
				.tickValues(d3.range(y2Min, y2Max,(y2Max-y2Min)/multipleNum));
			yAxis2
				.tickSize( -availableWidth, 0);
			...
			circles = wrap.selectAll('circle.ele')
				.attr('stroke','#009ee3');
			var rect1 = wrap.select('.nv-multibar .nv-group.nv-series-0')
						.style('transform','translate(0,-2px)');
			var rect2 = wrap.select('.nv-multibar .nv-group.nv-series-1')
						.style('transform','translate(0,1px)');
			rects = wrap.selectAll('rect.ele')
						.classed('selected',true);
			eles = wrap.selectAll('.ele');
				// .on('mouseover',function(d,i){
				...
				// })
			// var eleSum = eles.filter(function(d,i){
			//     console.log("ineles",d);
			//     return (d.series==0);
			// })
...
nv.models.activeChart = function() {
	"use strict";
	//============================================================
	// Public Variables with Default Settings
	//------------------------------------------------------------
	...
	var margin = {top: 30, right: 20, bottom: 50, left: 60},
		...
		showLegend = true,
		tooltips = true,
		tooltip = function(date, arg1, arg2, e, graph) {
			return '<h3>' + date + '</h3>' +
				'<p class="pro">新增唤醒人数</p>'+
				'<p class="num">' + arg1 + '</p>'+
				'<p class="pro">新增沉睡人数</p>'+
				'<p class="num">' + arg2+ '</p>';
		},
		x,
		y,
	...
	var x = d3.time.scale.utc(),
		...
		bars1 = nv.models.multiBar().stacked(false).groupSpacing(.2).yScale(yScale1),
		bars2 = nv.models.multiBar().stacked(false).groupSpacing(.2).yScale(yScale2),
		...
		legend = nv.models.legend().height(30),
		dispatch = d3.dispatch('tooltipShow', 'tooltipHide'),
		eles,elesIn,eleRe,eleSum,today;
	var showTooltip = function(e, offsetElement) {
		  eleSum = eles.filter(function(d,i){
				return (d.id==e.point.id);
			})
		...
		var left = e.pos[0] + ( offsetElement.offsetLeft || 0 ),
			...
			//x = xAxis.tickFormat()(lines1.x()(e.point, e.pointIndex)),
			y = arg1+arg2+((e.series.yAxis == 2) ? yAxis2 : yAxis1).tickFormat()(lines1.y()(e.point, e.pointIndex)),
			content = tooltip(x, arg1, arg2, e, chart);
			//y = y+eleSum[1].y+eleSum[2].y;
	...
	function chart(selection) {
		selection.each(function(data) {
		   ...
		   var extraValue1 = dataStack1.length ? dataStack1.map(function(a){return a.values}).reduce(function(a,b){
			...
			d3.transition(g.select('.x.axis'))
				.call(xAxis);
			var xTicks = g.select('.x.axis > g g').selectAll('g');
			...
			today =eles.filter(function(d,i){
				return (d.id=='13');
			})
			// console.log('sum',today)
			today
				.classed('today',true);
			legend.dispatch.on('stateChange', function(newState) {
				chart.update();
				...
				if (useVoronoi === true) {
					...
					if (clipVoronoi) {
						...
						pointClips
							...
							.attr('class', function(d,i){ return ('ele ele-'+i) });
					...
					pointPaths.enter().append('path')
						.attr('class', function(d,i) { return 'ele nv-path-'+i+' ele-'+i; });
...
nv.models.stackedAreaChart = function() {
	...
	d3.rebind(chart, stacked, 'x', 'y', 'size', 'xScale', 'yScale', 'xDomain', 'yDomain', 'xRange', 'yRange', 'sizeDomain', 'interactive', 'useVoronoi', 'offset', 'order', 'style', 'clipEdge', 'forceX', 'forceY'
	, 'forceSize', 'interpolate');
