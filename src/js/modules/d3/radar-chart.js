  ...
  chart: function() {
    ...
    function radar(selection) {
      selection.each(function(data) {
        ...
        function getHorizontalPositionReversal(i, range, factor){
          return getPosition(i, range, factor, Math.sin);
        }
        function getHorizontalPosition(i, range, factor){
          return getPosition(i, range, factor, function(x){
            return Math.sin(-x)
          });
        }
        ...
        polygon
          ...
          if(cfg.axisText) {
	        axis.select('circle')
	        	.attr('cx',function(d, i){ return getHorizontalPosition(i, cfg.w / 2, cfg.factorLegend); })
	        	.attr('cy',function(d, i){ return getVerticalPosition(i, cfg.h / 2, cfg.factorLegend); })
	        	.attr('r',9)
	        	.attr('fill',function(d, i){ return data[0].colors[i];	});
            axis.select('text')
              ...
              .attr('dy',function(d,i){
	            var p = getHorizontalPosition(i, 0.5);
	            return ((p < 0.4) ? '0.4em' : ((p > 0.6) ? '0.4em' : '0.5em'));
              })
              .attr('dx',function(d,i){
	            var p = getHorizontalPosition(i, 0.5);
	            return ((p < 0.4) ? '-0.3em' : ((p > 0.6) ? '-0.3em' : '0'));
              })
              .attr('fill','white')
        ...
        var label = levelGroups.selectAll('.label').data(function(levelFactor) {
          return d3.range(0, 1).map(function() { return levelFactor; });
        });
		//console.log(label);
        label.enter().append('text');
        label.exit().remove();
        label
          .text(function(levelFactor){return levelFactor/cfg.w*200;})
          .attr('class','labeltext')
/*
          .attr( 'x',function(levelFactor, i){ return (getHorizontalPosition(i, levelFactor)+getHorizontalPosition(i+1, levelFactor))/2; })
          .attr( 'y',function(levelFactor, i){ return (getVerticalPosition(i, levelFactor)+getVerticalPosition(i+1, levelFactor))/2; })
*/
          .attr('transform', function(levelFactor,i) {
	          if(levelFactor==(cfg.w/2))
	          	return 'translate(' + (cfg.w/2-levelFactor-10.5+(getHorizontalPositionReversal(i, levelFactor)+getHorizontalPositionReversal(i+1, levelFactor))/2) + ', ' + (cfg.h/2-levelFactor+2.5+(getVerticalPosition(i, levelFactor)+getVerticalPosition(i+1, levelFactor))/2) + ') rotate(-36)';

		   		return 'translate(' + (cfg.w/2-levelFactor-7+(getHorizontalPositionReversal(i, levelFactor)+getHorizontalPositionReversal(i+1, levelFactor))/2) + ', ' + (cfg.h/2-levelFactor+1+(getVerticalPosition(i, levelFactor)+getVerticalPosition(i+1, levelFactor))/2) + ') rotate(-36)';
          });
